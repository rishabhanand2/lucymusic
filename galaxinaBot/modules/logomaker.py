...
from galaxinaBot import OWNER_ID
...
async def lego(event):
 ...
 try:
    ...
    img = Image.open('./galaxinaBot/resources/blackbg.jpg')
    draw = ImageDraw.Draw(img)
    image_widthz, image_heightz = img.size
    pointsize = 500
    fillcolor = "gold"
    shadowcolor = "blue"
    font = ImageFont.truetype("./galaxinaBot/resources/Chopsic.otf", 330)
    ...
    fname2 = "LogoBygalaxina.png"
    img.save(fname2, "png")
    await tbot.send_file(event.chat_id, fname2, caption="💕 Made By @shivamdemon @alone_boy_xd_01")
...
async def lego(event):
 ...
 try:
    ...
    fillcolor = "white"
    shadowcolor = "blue"
    font = ImageFont.truetype("./galaxinaBot/resources/Maghrib.ttf", 1000)
    w, h = draw.textsize(text, font=font)
    h += int(h*0.21)
    image_width, image_height = img.size
    draw.text(((image_widthz-w)/2, (image_heightz-h)/2), text, font=font, fill=(255, 255, 255))
